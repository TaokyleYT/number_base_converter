...
def ConProg():
    ...
    debug = False
    number = input(
        'Input the number you want to convert, base above 10 is not being supported for this method so please enter numbers: '
    )
    base1 = int(
        input(
    ...
    if number == 'DEBUG':
        ...
        number = input('number?')
    if number.find('.') != -1:
        tempNum = convert(float(number), base1, base2)
    else:
        tempNum = convert(int(number), base1, base2)
    print(tempNum)
    if not debug:
        numSaver.append(
